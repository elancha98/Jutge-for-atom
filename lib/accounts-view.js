...
export default class AccountsView {
  ...
  _createLoginPanel() {
    ...
    fs.readFile(path.join(__dirname, '..', 'views', 'login.html'), "utf-8", (err, data) => {
      ...
      this._element.querySelector('#remember').addEventListener("change", () => {
        if (this._element.querySelector('#remember').checked) {
          const popup = this._element.querySelector('#popup');
          popup.style.opacity = 0;
          popup.style.visibility = "visible";
          (function fade() {
            let val = parseFloat(popup.style.opacity);
            if (!((val += .1) > 1)) {
              popup.style.opacity = val;
              requestAnimationFrame(fade);
            }
          })();
          console.log(this._element.querySelector('#popup').style.visibility);
          setTimeout( () => {
            const popup = this._element.querySelector('#popup');
            (function fade() {
